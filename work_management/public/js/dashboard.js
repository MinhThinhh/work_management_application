...
function updateDateDisplay() {
    ...
    if (navDateElement) {
        navDateElement.textContent = formatDateForNav(
            calendarState.currentDate
        );
    }

    // Update mini calendar date
    const miniCalendarDateElement = document.querySelector(
        "[data-mini-calendar-date]"
    );
    if (miniCalendarDateElement) {
        miniCalendarDateElement.textContent = formatDateForMiniCalendar(
            calendarState.currentDate
        );
    }
    ...
    if (calendarState.currentView === "day") {
        ...
        if (startOfWeek.getMonth() === endOfWeek.getMonth()) {
            ...
            return `${startOfWeek.getDate()} ${startOfWeek.toLocaleDateString(
                "vi-VN",
                { month: "short" }
            )} - ${endOfWeek.getDate()} ${endOfWeek.toLocaleDateString(
                "vi-VN",
...
function renderMiniCalendarDays() {
    const dayListElement = document.querySelector(
...
function renderMonthView(calendarElement) {
    ...
    for (let i = 0; i < firstDayOfWeek; i++) {
        ...
        const prevYear = currentMonth === 0 ? currentYear - 1 : currentYear;
        const date = new Date(prevYear, prevMonth, day);
        const dayElement = createMonthDayElement(
            day,
            "prev-month",
            isDateToday(date),
            date
    ...
    // Debug
    console.log(
        "Calendar events before adding to month view:",
        calendarState.events
    );
...
function addEventsToCalendar() {
    ...
    Object.keys(eventsByDate).forEach((date) => {
        const dayElement = document.querySelector(
            `.month-grid__day[data-date="${date}"]`
        );
...
function showEventDetails(event) {
    ...
    dateElement.innerHTML = `<strong>Ngày:</strong> ${formatDateForDisplay(
        event.date || event.due_date || event.start
    )}`;
    ...
    deleteButton.addEventListener("click", () => {
        if (confirm("Bạn có chắc chắn muốn xóa công việc này?")) {
            // Đóng modal và xóa overlay trước khi xóa
            document.body.removeChild(modal);
            removeOverlay();
            // Gọi hàm xóa mà không có confirm nữa
            deleteEventWithoutConfirm(event.id);
        }
    });
...
// Delete event with confirmation (for external use)
function deleteEvent(id) {
    ...
    // Gọi hàm xóa thực tế
    deleteEventWithoutConfirm(id);
}

// Delete event without confirmation (for internal use)
function deleteEventWithoutConfirm(id) {
    if (!id) return;
        ...
        .then((response) => {
            if (!response.ok) {
                ...
                const error = new Error(
                    "Lỗi khi xóa công việc: " + response.status
                );
            ...
            if (error.response) {
                ...
                error.response
                    ...
                    .catch((jsonError) => {
                        ...
                        showNotification(
                            "Lỗi khi xóa công việc: Không thể xóa công việc",
                            "error"
                        );
                    });
            } else {
                ...
                showNotification(
                    "Lỗi khi xóa công việc: " + error.message,
                    "error"
                );
            }
        });
...
function showDayEvents(date, events) {
    ...
    events.forEach((event) => {
        ...
        eventItem.addEventListener("click", () => {
            document.body.removeChild(modal);
            removeOverlay(); // Xóa overlay của modal hiện tại
            showEventDetails(event); // showEventDetails sẽ tạo overlay mới
        });
...
function setupEventHandlers() {
    ...
    const miniPrevButton = document.querySelector(
    ...
    const miniNextButton = document.querySelector(
    ...
    // Create event button
    const createEventButtons = document.querySelectorAll(
    ...
    fetch("/api/calendar/events", {
        ...
        .then((response) => {
            ...
            if (response.ok) {
                ...
                if (response.status === 401) {
                    ...
                    console.error(
                        "Lỗi xác thực (401) - Token không hợp lệ hoặc hết hạn"
                    );
                    throw new Error(
                    ...
                    throw new Error(
                        "Không thể tải dữ liệu: " + response.statusText
                    );
                }
            }
        })
        .then((data) => {
            ...
            // Nếu không có dữ liệu hợp lệ
                ...
                console.log(
                    "Không có định dạng dữ liệu hợp lệ, sử dụng mảng rỗng"
                );
                calendarState.events = [];
...
function removeOverlay() {
    ...
    if (overlay) {
        ...
        // Đảm bảo overlay được xóa hoàn toàn khỏi DOM sau một khoảng thời gian ngắn
        setTimeout(() => {
...
// Hàm helper để đảm bảo tất cả overlay được xóa
                ...
                fetch("/api/calendar/events", {
                    ...
                    .then((response) => {
                        ...
                        if (!response.ok) {
                            // Tạo một đối tượng lỗi với response để xử lý sau
                            const error = new Error(
                                "Lỗi khi tạo công việc: " + response.status
                            );
                            error.response = response;
                            throw error;
                        }
                        ...
                        if (error.response) {
                            ...
                            error.response
                                ...
                                .catch((jsonError) => {
                                    ...
                                    showNotification(
                                        "Lỗi khi tạo công việc: Dữ liệu không hợp lệ",
                                        "error"
                                    );
                                });
                        } else {
                            // Hiển thị thông báo lỗi chung
                            showNotification(
                                "Lỗi khi tạo công việc: " + error.message,
                                "error"
                            );
                        }
                    });
...
function renderWeekView(calendarElement) {
    ...
    // Debug
    console.log(
        "Calendar events before adding to week view:",
        calendarState.events
    );
...
function addEventsToWeekView() {
    ...
    console.log(
        "Thêm sự kiện vào chế độ xem tuần, tổng số:",
    ...
    weekEvents.forEach((event) => {
        try {
            ...
            const dayDiff = Math.floor(
                (eventDate - startOfWeek) / (24 * 60 * 60 * 1000)
            );
            ...
            const dayColumn = document.querySelector(
                `.week-view__day-column[data-day-index="${dayIndex}"]`
            );
            ...
            const dayContent = dayColumn.querySelector(
            ...
            eventElement.textContent =
                event.title || event.name || "Sự kiện không tên";
...
function renderDayView(calendarElement) {
    ...
    // Debug
    console.log(
        "Sự kiện trước khi thêm vào chế độ xem ngày:",
        calendarState.events
    );
...
function addEventsToDayView() {
    ...
    console.log(
        "Thêm sự kiện vào chế độ xem ngày, tổng số:",
        calendarState.events.length
    );
    ...
    if (dayEvents.length > 0) {
        showNotification(
            `Đã hiển thị ${dayEvents.length} sự kiện cho ngày này`,
            "success"
        );
    } else {
...
function kiemTraAPI() {
    ...
    fetch("/api/tasks", {
        ...
        .then((data) => {
            ...
            if (events.length > 0) {
                thongTinLoi.textContent += `\nSự kiện đầu tiên: ${JSON.stringify(
                    events[0]
                ).substring(0, 100)}...`;
            } else {
                thongTinLoi.textContent +=
